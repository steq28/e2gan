...
# Anti-overfitting Transformer Block
class AntiOverfittingTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch_size, channels, height * width).permute(2, 0, 1)  # Flatten spatial dims
        residual = x
        x = self.norm1(x)
        ...
        x = self.mlp(x)
        x = residual + x  # Residual connection
        x = x.permute(1, 2, 0).reshape(batch_size, channels, height, width)  # Reshape back
        return x
