...
class TextGuidedGenerator(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, text_embedding_dim=512, device='cuda'):
        super().__init__()
        
        # Inizializziamo CLIP per l'encoding del testo
        self.clip_model = CLIPTextModel.from_pretrained("openai/clip-vit-base-patch32").to(device)
        self.tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
        
        # Freeziamo i parametri di CLIP perché lo useremo solo per l'encoding
        ...
        # I due blocchi ResNet iniziali
        self.resnet_block1 = resnet.layer1
        self.resnet_block2 = resnet.layer2

        
        # Freeziamo anche i parametri di ResNet
        for param in self.downsample.parameters():
            param.requires_grad = False
        for param in self.resnet_block1.parameters():
            param.requires_grad = False
        for param in self.resnet_block2.parameters():
            param.requires_grad = False
        
        # DS (Downsampling) block con concatenazione dell'embedding testuale
        self.ds_block = nn.Sequential(
            nn.Conv2d(512 + text_embedding_dim, 512, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True)
        )
        
        # Transformer block con self-attention e cross-attention
    ...
    def encode_text(self, text):
        # Otteniamo il device da CLIP
        device = self.clip_model.device
        # Tokenizziamo e codifichiamo il testo usando CLIP
        tokens = self.tokenizer(text, padding=True, truncation=True, return_tensors="pt").to(device)
        text_features = self.clip_model(**tokens).last_hidden_state.mean(dim=1)
        return text_features

    def forward(self, x, text_prompt):
        batch_size = x.size(0)
        
        # Codifichiamo il prompt testuale con CLIP
        if isinstance(text_prompt, (list, tuple)):
            # Se abbiamo un batch di prompt, codifichiamo ciascuno
            text_embedding = self.encode_text(text_prompt)
        else:
            # Se abbiamo un singolo prompt, lo replichiamo per tutto il batch
        ...
        # Prepariamo l'embedding testuale per la concatenazione
        # Lo espandiamo per matchare le dimensioni spaziali dell'feature map
        text_embedding = text_embedding.unsqueeze(-1).unsqueeze(-1)  # [B, text_dim, 1, 1]
        text_embedding = text_embedding.expand(-1, -1, x.size(2), x.size(3))
        
        # Concateniamo le feature dell'immagine con l'embedding testuale
        ...
        # Self-attention e cross-attention nel transformer
        x_transformed = self.transformer(x_flat, x_flat)
        
        # Reshape back
        x = x_transformed.transpose(1, 2).view(b, c, h, w)
